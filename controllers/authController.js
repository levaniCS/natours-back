...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // returned data doesn't contain password so we added it to user object with 'select'
...
exports.isLoggedIn = async (req, res, next) => {
  if (req.cookies.jwt) {
    try {
      ...
      // 2) Check if user still exists  (SECURITY)
      ...
      // every pig template has access to locals variable
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  await user.save({ validateBeforeSave: false }); // Update resetToken to DB
  ...
  try {
    ...
    // Above two line only modifies data (not save)
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordResetToken = undefined;
  user.passwordResetExpires = undefined;
  // in this case we doesn't want to turn off validators
  //! (Validators will check if passwords are equal)
  await user.save();
